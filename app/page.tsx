...
export default function Home() {
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50">
      {/* Header */}
      <header className="bg-white border-b border-slate-200 px-3 py-2 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-2 min-w-0">
          <span className="text-xl flex-shrink-0">🏙️</span>
          <div className="min-w-0">
            <h1 className="text-sm font-bold text-slate-800 leading-tight">台北活動地圖</h1>
            <p className="text-xs text-slate-400 truncate">
              {loading && <span className="text-indigo-400 animate-pulse">載入中...</span>}
              {!loading && collectedAt && `更新於 ${new Date(collectedAt).toLocaleDateString('zh-TW')}`}
              {!loading && !collectedAt && '期間限定・展覽・市集・快閃'}
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden relative">
        {view === 'map' && (
          <>
            ...
            {/* Desktop sidebar */}
            <div className="hidden md:flex w-80 border-l border-slate-200 bg-white overflow-hidden flex-col">
              <EventDetail event={selectedEvent} onClose={() => setSelectedEvent(null)} />
            </div>
            {/* Mobile bottom sheet */}
            {selectedEvent && (
              <div className="md:hidden absolute bottom-0 left-0 right-0 bg-white border-t border-slate-200 shadow-lg max-h-[60vh] overflow-y-auto z-[1000]">
                <EventDetail event={selectedEvent} onClose={() => setSelectedEvent(null)} />
              </div>
            )}
          </>
